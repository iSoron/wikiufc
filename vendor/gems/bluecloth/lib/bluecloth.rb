...
class BlueCloth < String
	...
	def transform_code_blocks( str, rs )
		...
		str.gsub( CodeBlockRegexp ) {|block|
			...
			%{\n\n<pre><code>%s</code></pre>\n\n%s} %
				[ encode_code( outdent(codeblock).strip, rs ).rstrip, remainder ]
		}
	end
	...
	def form_paragraphs( str, rs )
		...
		rval = grafs.collect {|graf|
			...
			if rs.literal_blocks.key?( graf )
				rs.literal_blocks[ graf ]

			# Otherwise, wrap in <p> tags
			else
				result = apply_span_transforms(graf, rs).
					sub( /^[ ]*/, '<p>' ) + '</p>'
			end
		}.join( "\n\n" )
		...
		str.
			gsub( ReferenceImageRegexp ) {|match|
				...
				if rs.urls.key?( linkid )
					...
					# Build the tag
					result = %{<img src="%s" alt="%s" class="insert_image"} % [ url, alt ]
					if rs.titles.key?( linkid )
